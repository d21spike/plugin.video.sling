...
class Sling(object):
    ...
    def tryRecord(self):
        log('tryRecord(): Attempting to get record asset for channel %s @ %s' %
            (self.params['channel'], self.params['start']))

        channel_guid = self.params['channel']
        rec_start = int(self.params['start'])
        url_timestamp = datetime.datetime.fromtimestamp(rec_start).strftime(
            "%y%m%d") + datetime.datetime.fromtimestamp(rec_start).strftime("%H%M")
                        ...
                        for program in schedule['scheduleList']:
                            if 'schedule_start' in program:
                                if str(rec_start) in program['schedule_start']:
                                    log('tryRecord: Found %s' %
                                        program['title'])
                                    if 'external_id' in program:
                                        asset_id = program['external_id']
                                        asset_url = '%s/cms/publish3/asset/info/%s.json' % (
                                            self.endPoints['cms_url'], asset_id)
                                        log('tryRecord: Found %s, URL: %s' %
                                            (program['title'], asset_url))
